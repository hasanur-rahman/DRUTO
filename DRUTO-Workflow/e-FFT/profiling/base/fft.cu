...
__global__ void fft_kernel(const cuDoubleComplex* x, cuDoubleComplex* Y, uint32_t N, int logN)
{
    ...
    for (int s = 1; s <= logN; s++) {
        ...
        //printf("#Hasan# radian value: %.8lf\n ", -(double)3.14159265 * ((1.0 * j) / (1.0 * mh)));
        sincos((double)M_PI * ((1.0 * j) / (1.0 * mh)), &ti, &tr);
        cuDoubleComplex twiddle = make_cuDoubleComplex(tr, ti);

        cuDoubleComplex b = cuCmul(twiddle, Y[kj + mh]);
...
// Return value:
// none
void show_complex_gpu_vector(cuDoubleComplex* v, uint32_t N)
{
    printf("TOTAL PROCESSED SAMPLES: %u\n", N);
    printf("%s\n", "================================");
...
static const double complex sample_input[] = { 
    3.6 + 2.6I, 
    2.9 + 6.3I,
    5.6 + 4.0I,
    4.8 + 9.1I,
...
int setup_gpu(cuDoubleComplex** in, cuDoubleComplex** out, uint32_t N, int fill_with, int no_sample)
{
    ...
    // Allocate arrays of size N
    *in  = (cuDoubleComplex*) malloc(N * sizeof(cuDoubleComplex)); 
    CHECK_MALLOC(in, "input");
    *out = (cuDoubleComplex*) malloc(N * sizeof(cuDoubleComplex)); 
    CHECK_MALLOC(out, "output");

    for (size_t i = 0; i < N; i++) {
        (*in)[i] = make_cuDoubleComplex(fill_with, 0);
    }
    ...
    if (!no_sample) {
        for (size_t i = 0; i < ((N < sample_size) ? N : sample_size); i++) {
            double complex x = sample_input[i];
            (*in)[i] = make_cuDoubleComplex(creal(x), cimag(x));
        }
    }
...
int main(int argc, const char** argv)
{
    ...
    cuDoubleComplex* in_gpu;
    cuDoubleComplex* out_gpu;
