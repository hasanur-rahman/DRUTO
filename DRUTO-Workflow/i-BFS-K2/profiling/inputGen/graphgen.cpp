...
//    uniform_int_distribution<ulong> randNode(0, numNodes - 1);
    ...
    for (i = 0; i < numNodes; i++) {
        numEdges = (uint)((MAX_INIT_EDGES - MIN_EDGES + 1) + MIN_EDGES) * 0.70 ; //;rand() % (MAX_INIT_EDGES - MIN_EDGES + 1) + MIN_EDGES;
        for (j = 0; j < numEdges; j++) {
            nodeID = curNodeId; //randNode(gen);
            weight = (uint)((MAX_WEIGHT - MIN_WEIGHT + 1) + MIN_WEIGHT) * 0.70 ;//rand() % (MAX_WEIGHT - MIN_WEIGHT + 1) + MIN_WEIGHT;
