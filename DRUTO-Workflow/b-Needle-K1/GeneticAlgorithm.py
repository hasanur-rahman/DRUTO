...
class GeneticAlgorithm:
    ...
    def getActualBounds(self):
        ...
        for i in range(len(self.bounds)):
            if i == 0:
                bounds.append([self.bounds[i][0] * 16, self.bounds[i][1] * 16])
            else:
    ...
    def mutation(self):
        ...
        for i in range(self.pop_size):
            if self.pm > random.random():
                ...
                new_value = 0
                if mutation_index == 0:
                    new_value = random.randint(int(mutation_amount*-1), int(mutation_amount)) * 16 + org_value
                else:
                    new_value = random.randint(int(mutation_amount*-1), int(mutation_amount)) + org_value                

                actualBounds = self.getActualBounds()
                if new_value > actualBounds[mutation_index][1]:
                    new_value = actualBounds[mutation_index][1]
                elif new_value < actualBounds[mutation_index][0]:
                    new_value = actualBounds[mutation_index][0]
                chm.x[mutation_index] = new_value

                print("after mutation input: ", chm.x)
    ...
    def selectR(self):
        ...
        if mn < 0:
            for i in range(self.pop_size):
                self.pop[i].y = self.pop[i].y + (-1) * mn
        v = []
        v.append(self.pop[maxI])
