...
#staticInstIndex = "fiInstIndex=83"
...
optFlags = ""
#############################################################################
makeCommand1 = flagHeader + " pathfinder.cu -o pathfinder.o" + ktraceFlag 
makeCommand2 = ""
makeCommand3 = ""
makeCommand4 = ""
...
def compileInjectionPass():
	...
	os.system("nvcc -arch=sm_30 injection_runtime.o -lcudart -lnvToolsExt" + linkList + " -o " + outputExeFile + " -O0 " + linkFlags)
...
def groupProfileLines():
	profileLineList = []
	global memDynamicKernelIndices
	...
	for line in profileLineList:
		...
		if dynamicKernelIndex not in profileDic:
			memDynamicKernelIndices.append(dynamicKernelIndex)
...
def generateFiPointFromGroupedLines():
	...
	totalIters = random.randint(1, 10)
	countIters = 0	
...
#			break
	...
	while countIters < totalIters :
		selectedDynamicKernelIndex = memDynamicKernelIndices[random.randint(0, groupSize-1)]
		countIters += 1	
	...
	print "input: " + `inputParameters` + " ***[GPGPU-BAMBOO]*** Injecting to threadID:" + `fiThreadIndex` + ", fiInstCount:" + `fiInstCount` + ", fiDynKernel:" + `fiDynamicKernelIndex` + ", fiStaticKernel:" + `fiStaticKernelIndex`
