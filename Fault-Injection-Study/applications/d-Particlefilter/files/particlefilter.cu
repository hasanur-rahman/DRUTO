...
void particleFilter(int *I, int IszX, int IszY, int Nfr, int *seed,
    ...
    for (k = 1; k < Nfr; k++) {
        ...
        for (x = 0; x < Nparticles; x++) {
            arrayX[x] = arrayX[x] + 1.0 + 5.0 * x; //randn(seed, x);
            arrayY[x] = arrayY[x] - 2.0 + 2.0 * x; //randn(seed, x);
        }
	...
	FILE *fpo = fopen("output.txt", "a");
	fprintf(fpo, "XE: %.2lf\n", xe);
	fprintf(fpo, "YE: %.2lf\n", ye);
	fprintf(fpo, "distance: %.2lf\n", distance);
	fclose(fpo);
        ...
        double u1 = (1 / ((double)(Nparticles))) * 0.5; //randu(seed, 0);
        for (x = 0; x < Nparticles; x++) {
